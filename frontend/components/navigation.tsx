  ...
  const NavLink = ({ href, label, icon: Icon, badge }: any) => {
    ...
    return (
      ...
      >
        ...
        <span>{label}</span>
        {badge && (
          <Badge className="ml-auto bg-red-500 hover:bg-red-600 text-xs">{badge}</Badge>
        )}
      </Link>
    )
  }
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-bold text-xl text-gray-900">ChineseMaster</span>
          </Link>
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center gap-1">
                  <Trophy className="h-4 w-4 mr-1" /> Thành tích <ChevronDown className="h-4 w-4" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" sideOffset={8} className="w-56 p-1">
                ...
                <DropdownMenuSeparator />
                <div className="flex items-center gap-2 px-2 py-1.5 text-sm text-yellow-600">
                  <Star className="h-4 w-4" /> 2,450 XP
                </div>
              </DropdownMenuContent>
            </DropdownMenu>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center gap-1">
                  <Users className="h-4 w-4 mr-1" /> Cộng đồng <ChevronDown className="h-4 w-4" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="start" sideOffset={8} className="w-56 p-1">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="sm" className="flex items-center gap-2">
                  <User className="h-4 w-4" /> Hồ sơ <ChevronDown className="h-4 w-4" />
                </Button>
              </DropdownMenuTrigger>
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side="right" className="w-80">
                <SheetHeader>
                  ...
                  <SheetDescription>Điều hướng ứng dụng học tiếng Trung</SheetDescription>
                </SheetHeader>
